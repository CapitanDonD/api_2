...
def shorten_link(bitly_headers, url):
    payload =  {"long_url": url}
    attribute_url = "https://api-ssl.bitly.com/v4/bitlinks"
    response = requests.post(attribute_url, headers=bitly_headers, json=payload)
    response.raise_for_status()
    return response.json()["id"]


def count_clicks(bitly_headers, bitlink):
    ...
    response = requests.get(attribute_url, headers=bitly_headers)
    response.raise_for_status()
...
if __name__ == "__main__":
    ...
    if is_bitlink(authorization, args.url_or_bitly):
        try:
            clicks_bitlink = count_clicks(bitly_headers, args.url_or_bitly)
            print(f"Колличество кликов по ссылке: {clicks_bitlink}")
        except requests.exceptions.HTTPError:
            print("Ошибка при подсчётывании кликов")
    else:
        try:
            bitlink = shorten_link(bitly_headers, args.url_or_bitly)
            print(bitlink)
        except requests.exceptions.HTTPError:
